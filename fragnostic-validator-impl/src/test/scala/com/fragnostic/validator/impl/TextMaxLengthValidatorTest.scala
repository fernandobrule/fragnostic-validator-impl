...
class TextMaxLengthValidatorTest extends AgnosticLifeCycleValidatorTest {
  ...
  describe("***Text Max Length Validator Test***") {
    ...
    val messages: Map[String, String] = Map(
      MSG_TEXT_MAX_LENGTH_VALIDATOR_TEXT_IS_TOO_LONG -> validatorI18n.getString(localePtBr, MSG_TEXT_MAX_LENGTH_VALIDATOR_TEXT_IS_TOO_LONG))
    ...
    it("Can Validate Text Max Length That Compliant") {
      ...
      val text = "abcde"
      val validation: Validated[String] = textMaxLengthValidator.validate(localePtBr, domain, text, params, messages)
    ...
    it("Can Validate Text Max Length That Does Not Compliant") {
      ...
      val text = "abcdef"
      val msgTooLong = validatorI18n.getFormattedString(localePtBr, MSG_TEXT_MAX_LENGTH_VALIDATOR_TEXT_IS_TOO_LONG, List(text.length.toString, maxLength))
      val messages: Map[String, String] = Map(
        MSG_TEXT_MAX_LENGTH_VALIDATOR_TEXT_IS_TOO_LONG -> msgTooLong //
      )

      val nel = textMaxLengthValidator.validate(localePtBr, domain, text, params, messages) fold (
        error => error,
