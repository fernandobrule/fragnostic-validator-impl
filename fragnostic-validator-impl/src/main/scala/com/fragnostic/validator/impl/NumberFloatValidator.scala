        ...
        handleFloat(CONF_MIN_VALUE, domain, params) fold (
          ...
          minValue => {
            if (someNumber < minValue) {
              getMessage(locale, MSG_NUMBER_FLOAT_VALIDATOR_NUMBER_IS_TOO_SHORT, messages).failureNel
            } else if (someNumber > maxValue) {
              getMessage(locale, MSG_NUMBER_FLOAT_VALIDATOR_NUMBER_IS_TOO_LONG, messages).failureNel
            } else {
